describe("Test login and loads dashboard with linked accounts", () => {
  it("Logs in an already established user", () => {
    // visit login page
    cy.visit("/login");

    // fill in input fields
    cy.get("input[name=email]").type("admin_hasAccounts");
    cy.get("input[name=password]").type("admin");

    // check if input fields have correct values
    cy.get("input[name=email]").should("have.value", "admin_hasAccounts");
    ...
    // budget widget should display budget

    // net worth widget should display net worth

    // transactions widget should display transactions
